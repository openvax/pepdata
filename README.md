...
One aspect of computational immunology is modeling the properties of *peptides* (short strings of amino acids). Peptides can arise as substrings of some larger protein, naturally occurring small proteins, or be synthesized for therapeutic purposes. We can investigate whether a peptide is presented on the cell surface for immune recognition by [MHC molecules](http://en.wikipedia.org/wiki/Major_histocompatibility_complex), in which case we call it an *epitope*. To make useful predictions (i.e. "which peptides should go in this vaccine?") we need to partition the combinatorial space of peptides into classes such as epitopes vs. non-epitope or immunogenic vs. non-immunogenic (do [white blood cells](http://en.wikipedia.org/wiki/Lymphocyte) respond?). A fruitful approach to capturing such distinctions is to collect a large volume of data about peptides and build statistical models of their properties. This library (`epitopes`) seeks to aid you in that process by providing simple Python/NumPy/Pandas interfaces to commonly used immunology and bioninformatics datasets. 
